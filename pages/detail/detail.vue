<template>
	<view class="detailContainer">
		<view class="header">
			...
			<text>网易严选 </text>
			<view class="shopCart" >
				<icon class="iconfont icon-gouwuche2" @click="goShopCart"></icon>
...
<script>
	export default {
		...
		methods:{
			async getShopDetail() {
				this.shopDetail = await this.$http("/getShopDetail",{id:this.shopId});
			},
			async addShopCart() {
				await this.$http("/addShopCart",{id:this.shopId})
				uni.showToast({
					title:"添加成功",
					duration:1300,
					icon:"none",
